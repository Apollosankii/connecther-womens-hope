    ...
    private fun saveUserInfo() {
        ...
        if (SupabaseData.isConfigured()) {
            lifecycleScope.launch {
                val ok = SupabaseData.updateUserProfile(firstName, lastName, phone, email, occupation)
                if (ok) {
                    Toast.makeText(this@SettingsActivity, "User info updated successfully", Toast.LENGTH_SHORT).show()
                    CurrentUser.getUser()?.let { u ->
                        CurrentUser.setUser(u.copy(
                            first_name = firstName.ifBlank { u.first_name },
                            last_name = lastName.ifBlank { u.last_name },
                            phoneNumber = phone.ifBlank { u.phoneNumber },
                            occupation = occupation.ifBlank { null } ?: u.occupation
                        ))
                    }
                } else {
                    Toast.makeText(this@SettingsActivity, "Failed to update user info", Toast.LENGTH_SHORT).show()
                }
            }
        } else {
