...
def _plan_to_dict(row):
    ...
    if isinstance(features, list):
        ...
        feats = []
    cper = row.get('connects_per_period')
    try:
        cper_i = int(cper) if cper is not None and str(cper).strip() != '' else None
    ...
    return {
        ...
        'connects_per_period': cper_i,
        'connects_period_rule': row.get('connects_period_rule') or 'subscription_term',
    }


def get_platform_settings(session):
    """Singleton row id=1: free_tier_connects for new accounts."""
    data = _req(session, 'GET', '/platform_settings?id=eq.1')
    if not data or not isinstance(data, list) or len(data) == 0:
        return {'id': 1, 'free_tier_connects': 5}
    return data[0]


def update_platform_settings(session, free_tier_connects: int):
    """Update free tier default (new signups via trigger; existing users keep their granted value until admin edits user)."""
    from datetime import datetime, timezone

    row = {
        'free_tier_connects': max(0, int(free_tier_connects)),
        'updated_at': datetime.now(timezone.utc).isoformat(),
    }
    res = _req_write(session, 'PATCH', '/platform_settings?id=eq.1', json_data=row)
    return _patch_ok(res)
...
def add_subscription_plan(session, payload):
    ...
    row = {
        ...
        'connects_period_rule': payload.get('connects_period_rule') or 'subscription_term',
    }
    cpp = payload.get('connects_per_period')
    if cpp is not None and str(cpp).strip() != '':
        try:
            row['connects_per_period'] = max(0, int(cpp))
        except (TypeError, ValueError):
            row['connects_per_period'] = None
    else:
        row['connects_per_period'] = None
...
def update_subscription_plan(session, plan_id, payload):
    ...
    for k, v in payload.items():
        if k not in allowed:
            continue
        if v is None and k != 'connects_per_period':
            continue
        row[k] = v
    if not row:
        return False
...
def list_user_plan_subscriptions(session, status_filter=None, plan_filter=None):
    ...
    for r in data:
        ...
        remaining = (cg - cu) if cg is not None else None
        connects_cell = '—'
        if remaining is not None:
            connects_cell = f"{remaining} left ({cu}/{cg} used)"
        elif p.get('connects_limit_enabled'):
            connects_cell = 'Not set (edit plan or subscription)'
        else:
            connects_cell = 'Unlimited'
        out.append({
            'id': r.get('id'),
            ...
            'connects_display': connects_cell,
        })
    return out


def grant_user_plan_subscription(
    session,
    user_id,
    plan_id,
    started_at,
    ...
    notes=None,
    connects_granted_override=None,
):
    """Grant a plan to a user. user_id=users.id (integer), plan_id=subscription_plans.id.
    Initializes connects_granted/used when plan has connects_limit_enabled and a quota.
    Returns (True, None) on success, (False, 'reason') on failure."""
    plan = get_subscription_plan(session, plan_id)
    if not plan:
        return False, 'plan_not_found'
    if plan.get('connects_limit_enabled') and plan.get('connects_per_period') is None:
        return False, 'plan_connect_quota_missing'
    row = {
        'user_id': int(user_id),
        'plan_id': int(plan_id),
        'status': 'active',
    ...
    if plan.get('connects_limit_enabled') and plan.get('connects_per_period') is not None:
        base_granted = int(plan['connects_per_period'])
        if connects_granted_override is not None and str(connects_granted_override).strip() != '':
            try:
                base_granted = max(0, int(connects_granted_override))
            except (TypeError, ValueError):
                pass
        row['connects_granted'] = base_granted
        row['connects_used'] = 0
        row['connects_period_started_at'] = started_at
    else:
        row['connects_granted'] = None
        row['connects_used'] = 0
        row['connects_period_started_at'] = None
    res = _req_write(session, 'POST', '/user_plan_subscriptions', json_data=row)
    ok = res is not None and (isinstance(res, list) or isinstance(res, dict))
    return (True, None) if ok else (False, 'write_failed')
...
def get_user_plan_subscription(session, sub_id):
    ...
    plans = _req(session, 'GET', '/subscription_plans', params={
        ...
        'select': (
            'id,name,price,currency,duration_type,duration_value,connects_limit_enabled,'
            'connects_per_period,connects_period_rule'
        ),
    })
    ...
    cu = int(r.get('connects_used') or 0)
    remaining = (cg - cu) if cg is not None else None
    rule = p.get('connects_period_rule') or 'subscription_term'
    rule_labels = {
        'subscription_term': 'Full subscription term (start → expiry date)',
        'calendar_month': 'Resets each calendar month',
    ...
    return {
        ...
        'plan_connects_limit_enabled': bool(p.get('connects_limit_enabled', False)),
        'plan_connects_per_period': p.get('connects_per_period'),
        'plan_connects_period_rule': rule,
        'plan_connects_period_rule_label': rule_labels.get(rule, rule),
        'status': r.get('status'),
...
def update_user_plan_subscription(session, sub_id, payload):
    """Update subscription row including connect balance fields (null connects_granted = unlimited)."""
    allowed = (
    ...
    for k, v in payload.items():
        if k not in allowed:
            continue
        if k in ('connects_granted', 'connects_period_started_at') or v is not None:
            row[k] = v
    if not row:
        return False
    res = _req_write(session, 'PATCH', f'/user_plan_subscriptions?id=eq.{sub_id}', json_data=row)
