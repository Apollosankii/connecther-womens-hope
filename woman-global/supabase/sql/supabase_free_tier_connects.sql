...
CREATE TRIGGER tr_users_init_free_tier
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.users_init_free_tier();
...
BEGIN
  ...
  -- No active subscription: free tier on users (effective cap never exceeds platform_settings.free_tier_connects)
...
BEGIN
  ...
  -- No active subscription: show free tier (same cap as consume_client_subscription_connect)
  v_ft_default := COALESCE((SELECT ps.free_tier_connects FROM public.platform_settings ps WHERE ps.id = 1 LIMIT 1), 0);
