-- Block a seeker from opening a second booking with the same provider while an overlapping
-- engagement exists (pending/accepted request or incomplete job).
...
BEGIN
  ...
  -- Already have a live request or active job with this provider — avoid double booking.
  IF EXISTS (
