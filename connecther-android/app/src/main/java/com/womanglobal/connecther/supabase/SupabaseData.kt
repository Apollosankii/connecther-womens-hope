...
object SupabaseData {
    ...
    /**
     * Returns true if the exception indicates the user already exists (unique constraint on clerk_user_id).
    ...
    /**
     * Register FCM token with Supabase (Clerk path). Call after Clerk sign-in.
    ...
    suspend fun upsertFcmToken(regToken: String, deviceId: String): Boolean {
        ...
        return runCatching {
            if (!SupabaseClientProvider.ensureClerkSession()) return@runCatching false
    ...
    /**
     * Fetches user profile by Clerk user ID. Requires Clerk session (JWT) for RLS.
     */
    suspend fun getUserProfile(clerkUserId: String): User? {
        if (!SupabaseClientProvider.ensureClerkSession()) return null
        val client = SupabaseClientProvider.client
        return runCatching {
    ...
    /**
     * Inserts a new user during onboarding. Requires Clerk session (JWT) for RLS.
     * When Supabase is not configured (placeholder URL), returns without inserting (allows local-only onboarding).
     * @throws Exception if Clerk session cannot be established or insert fails
     */
    suspend fun insertUser(payload: InsertUserPayload) {
        ...
        if (!SupabaseClientProvider.ensureClerkSession()) {
            throw IllegalStateException("Clerk session not available. Ensure Clerk JWT template 'supabase' is configured.")
        }
        val client = SupabaseClientProvider.client
     ...
     * Requires Clerk session (JWT). Admin approves from provider_applications.
    ...
    suspend fun submitProviderApplication(
        ...
        if (!SupabaseClientProvider.ensureClerkSession()) {
            android.util.Log.e("SupabaseData", "submitProviderApplication: Clerk session not available")
            return false
        }
        val client = SupabaseClientProvider.client
    ...
    suspend fun upsertLiveLocation(latitude: Double, longitude: Double): Boolean {
        if (!SupabaseClientProvider.ensureClerkSession()) return false
        val client = SupabaseClientProvider.client
    ...
    suspend fun getPendingJobs(): List<Job> {
        ...
        if (!SupabaseClientProvider.ensureClerkSession()) return emptyList()
        return runCatching {
